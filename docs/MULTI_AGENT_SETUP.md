...
**Scope an agent to a Drive folder.** Permissions → the agent → the **drive** service → **Path rules**. Set the default to **blocked**, then add one rule granting `read` or `write` on the folder (paste the folder's Drive URL; the id is extracted). A rule covers the folder **and everything beneath it**: every Drive tool resolves the file's ancestry and applies the nearest rule, a file with several parents is refused if any chain is blocked, search results outside the folder are dropped, and moves into or out of the folder need write on both ends. With the default blocked, a listing without a folder shows the granted folders themselves, so the agent can see what it may enter. The same rules govern Drive attachments on Gmail tools, so a file cannot be read by attaching it to a draft.

**The agent is told its limits.** A restricted agent does not have to discover its boundaries by being refused. The MCP announces them in three places: the `instructions` returned at connect (Claude, Claude Code, and Cowork keep it in the model's context), the descriptions of the affected tools in `tools/list` ("Scopes you can reach: lva (default)", "Limited to project LVA; project_id is filled in", "Limited to Drive folder /proj (read)"), and a `limits` object on `whoami` for anything that wants the machine-readable version. An unrestricted agent sees none of this.

**Other services narrow the same way.** Hermeneutix can be pinned to one project when it is added to an agent (Permissions → **Add service** → Hermeneutix → pick the project; **All projects** leaves it unscoped, and the choice is editable later in the service details). A pinned instance fills `project_id` in automatically, refuses any other project, and verifies that every meeting, instance, and conversation it returns belongs to that project.
