...
Tests the full user onboarding flow: qualification → approval → Gmail OAuth → MiniMax key → bot token → notify bot → agent deployment → ping → browser.

## Prerequisites

| Requirement | Details |
|-------------|---------|
| Playwright MCP | Must be connected (`Codex mcp list` shows `playwright: ✓ Connected`) |
...
- Bot asks user to message `@AgentHelmApprovalsBot`
- Use `page.locator('a[href="#8781774032"]').click()` to navigate to it
...
Navigate to the agent bot (`@fsaintPA_bot`):
- Click `a[href="#8366850213"]` in the chatlist, or use `page.locator('a[href="#8366850213"]').click()`
- Send: `hello`
- **Pass:** agent replies within ~30s with any coherent response (e.g. "Hello! How can I help you today?")
- **Fail:** no response after 60s, or error message like "Something went wrong"
...
### 11. Browser test — agent can use browser tool

Still in the agent bot chat, send:
```
...
- Tool call → AgentHelm policy check → browser → response pipeline is working end-to-end
