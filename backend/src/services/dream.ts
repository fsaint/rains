...
7. Update THAT SCOPE'S index entry to reflect: the key people, projects, and notes in that scope, and a brief reflection on what you have learned recently. memory_get_root returns one index per scope — update each one from its own scope's contents only. Edit the index with memory_update's section or append operations rather than resending the whole content, and pass the version you read as if_version. A VERSION_CONFLICT means another agent wrote in between: re-read and re-apply your change rather than overwriting.
